...
async function getData(req, res, next) {
  ...
  try {
    let serialId = Mongo.ObjectId(req.params.serialId);

    let result = await Mongo.db.collection("batch").findOne({ _id: serialId });
    if (result) {
      if (result.status === "running") {
        let lastEvent = await Mongo.db.collection("events").findOne({ "event_data.batch_id": serialId }, { sort: { event_time: -1 } });
        if (lastEvent) {
          let partsOk = 0;
          let partsNg = 0;
      ...
      let err = new Error(`Batch with _id ${serialId} does not exist`);
      err.status = 400;
      throw err;
    }
  }
