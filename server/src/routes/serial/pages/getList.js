...
function getMatch(reqParams, reqQuery) {
  ...
  if (Boolean(reqQuery) && Object.keys(reqQuery).length > 0) {
    if (
      reqQuery.hasOwnProperty("min_event_time") ||
      reqQuery.hasOwnProperty("max_event_time")
    ) {
      match = {
        "event_data.window_end_time": {},
      };
      if (reqQuery.hasOwnProperty("min_event_time")) {
        if (!isIsoDate(reqQuery["min_event_time"])) {
          raiseError("Invalid min_event_time. Valid iso date is required.");
        }
        // match.start_time["$gte"] = new Date(reqQuery.min_event_time);
        match["event_data.window_end_time"]["$gte"] = new Date(
          reqQuery.min_event_time
        );
      }
      if (reqQuery.hasOwnProperty("max_event_time")) {
        if (!isIsoDate(reqQuery["max_event_time"])) {
          raiseError("Invalid max_event_time. Valid iso date is required.");
        }
        // match.start_time["$lte"] = new Date(reqQuery.max_event_time);
        match["event_data.window_end_time"]["$lte"] = new Date(
          reqQuery.max_event_time
        );
      }
    }
    ...
    if (!Mongo.ObjectId.isValid(reqParams["stationId"])) {
      raiseError(
        `Invalid station ${reqParams["stationId"]}. Valid ObjectId is required.`
      );
    }
...
async function getList(req, res, next) {
  try {
    ...
    const inspectionIdsList = (
      (await Mongo.db
        .collection(collection)
        .find(dateMatch)
        // .find()
        .project({ _id: 0, "event_data.inspection_id": 1 })
        .toArray()) ?? []
    ).map((el) => el.event_data.inspection_id);
